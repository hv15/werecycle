...
package com.google.android.apps.recyclefinder.services;

import static com.google.android.apps.recyclefinder.Constants.RESUME_TRACK_EXTRA_NAME;

import com.google.android.apps.recyclefinder.Constants;
import com.google.android.apps.recyclefinder.TrackDetailActivity;
import com.google.android.apps.recyclefinder.content.DescriptionGeneratorImpl;
import com.google.android.apps.recyclefinder.content.RecycleFinderLocation;
import com.google.android.apps.recyclefinder.content.RecycleFinderProvider;
import com.google.android.apps.recyclefinder.content.RecycleFinderProviderUtils;
import com.google.android.apps.recyclefinder.content.Sensor;
import com.google.android.apps.recyclefinder.content.Sensor.SensorDataSet;
import com.google.android.apps.recyclefinder.content.Track;
import com.google.android.apps.recyclefinder.content.Waypoint;
import com.google.android.apps.recyclefinder.content.WaypointCreationRequest;
import com.google.android.apps.recyclefinder.content.WaypointCreationRequest.WaypointType;
import com.google.android.apps.recyclefinder.services.sensors.SensorManager;
import com.google.android.apps.recyclefinder.services.sensors.SensorManagerFactory;
import com.google.android.apps.recyclefinder.services.tasks.AnnouncementPeriodicTaskFactory;
import com.google.android.apps.recyclefinder.services.tasks.PeriodicTaskExecutor;
import com.google.android.apps.recyclefinder.services.tasks.SplitPeriodicTaskFactory;
import com.google.android.apps.recyclefinder.stats.TripStatistics;
import com.google.android.apps.recyclefinder.stats.TripStatisticsUpdater;
import com.google.android.apps.recyclefinder.util.IntentUtils;
import com.google.android.apps.recyclefinder.util.LocationUtils;
import com.google.android.apps.recyclefinder.util.PreferencesUtils;
import com.google.android.apps.recyclefinder.util.TrackNameUtils;
 ...
 * points. Track points are saved to the {@link RecycleFinderProvider}.
 * 
...
public class TrackRecordingService extends Service {
  ...
  private RecycleFinderProviderUtils recycleFinderProviderUtils;
  private RecycleFinderLocationManager recycleFinderLocationManager;
  ...
  private LocationListener locationListener = new LocationListener() {
    ...
    public void onLocationChanged(final Location location) {
      if (recycleFinderLocationManager == null || executorService == null
          || !recycleFinderLocationManager.isAllowed() || executorService.isShutdown()
   ...
   * allow both RecycleFinder and third party apps to invoke it. For the onCreate
   * callback, we cannot tell whether the caller is RecycleFinder or a third party
   * app, thus it cannot start/stop a recording or write/update RecycleFinder
   * database.
   */
  @Override
  public void onCreate() {
    super.onCreate();
    context = this;
    recycleFinderProviderUtils = RecycleFinderProviderUtils.Factory.get(this);
    recycleFinderLocationManager = new RecycleFinderLocationManager(this);
     ...
     */
    Track track = recycleFinderProviderUtils.getTrack(recordingTrackId);
    if (track != null) {
      restartTrack(track);
    } else {
      if (isRecording()) {
  ...
  /*
   * Note that this service, through the AndroidManifest.xml, is configured to
   * allow both RecycleFinder and third party apps to invoke it. For the onStart
   * callback, we cannot tell whether the caller is RecycleFinder or a third party
   * app, thus it cannot start/stop a recording or write/update RecycleFinder
   * database.
   */
  ...
  /*
   * Note that this service, through the AndroidManifest.xml, is configured to
   * allow both RecycleFinder and third party apps to invoke it. For the
   * onStartCommand callback, we cannot tell whether the caller is RecycleFinder or a
   * third party app, thus it cannot start/stop a recording or write/update
   * RecycleFinder database.
   */
  @Override
  ...
  public void onDestroy() {
    ...
    recycleFinderProviderUtils = null;
    recycleFinderLocationManager.close();
    recycleFinderLocationManager = null;
  ...
  public long insertWaypoint(WaypointCreationRequest waypointCreationRequest) {
    ...
    if (waypointCreationRequest.getName() != null) {
      ...
      int nextWaypointNumber = recycleFinderProviderUtils.getNextWaypointNumber(
    ...
    Uri uri = recycleFinderProviderUtils.insertWaypoint(waypoint);
  ...
  private long startNewTrack() {
    ...
    Uri uri = recycleFinderProviderUtils.insertTrack(track);
    ...
    recycleFinderProviderUtils.updateTrack(track);
  ...
  private void restartTrack(Track track) {
    ...
    Waypoint waypoint = recycleFinderProviderUtils.getLastStatisticsWaypoint(recordingTrackId);
    ...
    try {
      ...
      cursor = recycleFinderProviderUtils.getTrackPointCursor(
      ...
      if (cursor == null) {
        ...
        if (cursor.moveToLast()) {
          do {
            Location location = recycleFinderProviderUtils.createTrackPoint(cursor);
  ...
  private void endCurrentTrack() {
    ...
    // Update database
    Track track = recycleFinderProviderUtils.getTrack(trackId);
    if (track != null && !paused) {
      insertLocation(track, lastLocation, getLastValidTrackPointInCurrentSegment(trackId));
      updateRecordingTrack(track, recycleFinderProviderUtils.getLastTrackPointId(trackId), false);
    }
  ...
  private Location getLastValidTrackPointInCurrentSegment(long trackId) {
    ...
    return recycleFinderProviderUtils.getLastValidTrackPoint(trackId);
  }
  ...
  private void onLocationChangedAsync(Location location) {
    try {
      ...
      if (sensorDataSet != null) {
        location = new RecycleFinderLocation(location, sensorDataSet);
      }
  ...
  private void insertLocation(Track track, Location location, Location lastValidTrackPoint) {
    ...
    try {
      Uri uri = recycleFinderProviderUtils.insertTrackPoint(location, track.getId());
  ...
  private void registerLocationListener() {
    ...
    if (recycleFinderLocationManager == null) {
      Log.e(TAG, "locationManager is null.");
      return;
    }
    try {
      long interval = locationListenerPolicy.getDesiredPollingInterval();
      recycleFinderLocationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, interval,
  ...
  private void unregisterLocationListener() {
    ...
    recycleFinderLocationManager.removeUpdates(locationListener);
  }
  ...
  private void showNotification() {
    if (isRecording() && !isPaused()) {
      ...
      NotificationCompat.Builder builder = new NotificationCompat.Builder(this).setContentIntent(
          ...
          .setContentTitle(getString(R.string.recyclefinder_app_name)).setOngoing(true)
          .setSmallIcon(R.drawable.recyclefinder_notification_icon).setWhen(System.currentTimeMillis());
