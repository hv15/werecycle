...
import os
from recyclefinder.parser import StringsParser
import recyclefinder.history 
...
class Validator(object):
  ...
  def __init__(self, languages):
    ...
    for lang, lang_dir in languages.iteritems():
      ...
      recyclefinder.history.FillMercurialRevisions(filename, parsed_file)
  ...
  def _ValidateOutdatedKeys(self):
    ...
    for lang, file in self._langs.iteritems():
      ...
      for key, str in file.iteritems():
        ...
        master_rev = reduce(
            lambda r1, r2: recyclefinder.history.NewestRevision(master_file, r1, r2),
            master_revs)
        lang_rev = reduce(
            lambda r1, r2: recyclefinder.history.NewestRevision(lang_file, r1, r2),
            lang_revs)

        # If the master version is newer than the lang version
        if recyclefinder.history.DoesRevisionSuperceed(lang_file, master_rev, lang_rev):
          outdated.append(key)
