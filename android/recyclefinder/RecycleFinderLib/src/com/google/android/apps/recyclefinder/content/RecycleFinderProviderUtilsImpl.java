...
/**
 * {@link RecycleFinderProviderUtils} implementation.
 * 
 * @author Leif Hendrik Wilden
 */
public class RecycleFinderProviderUtilsImpl implements RecycleFinderProviderUtils {

  private static final String TAG = RecycleFinderProviderUtilsImpl.class.getSimpleName();

  private final ContentResolver contentResolver;
  private int defaultCursorBatchSize = 2000;

  public RecycleFinderProviderUtilsImpl(ContentResolver contentResolver) {
  ...
  public Location createTrackPoint(Cursor cursor) {
    Location location = new RecycleFinderLocation("");
  ...
  private ContentValues createContentValues(Location location, long trackId) {
    ...
    if (location instanceof RecycleFinderLocation) {
      RecycleFinderLocation recycleFinderLocation = (RecycleFinderLocation) location;
      if (recycleFinderLocation.getSensorDataSet() != null) {
        values.put(TrackPointsColumns.SENSOR, recycleFinderLocation.getSensorDataSet().toByteArray());
      }
    }
    return values;
  }
  ...
  private void fillTrackPoint(Cursor cursor, CachedTrackPointsIndexes indexes, Location location) {
    ...
    if (location instanceof RecycleFinderLocation && !cursor.isNull(indexes.sensorIndex)) {
      RecycleFinderLocation recycleFinderLocation = (RecycleFinderLocation) location;
      try {
        recycleFinderLocation.setSensorDataSet(
